...
if getattr(sys, "frozen", False):
    ...
    v2_data = installed_data_root()
    BUNDLE_DIR = materialize_runtime_assets(v2_data) if v2_data else Path(getattr(sys, "_MEIPASS", APP_DIR))
    RESOURCE_DIR = v2_data if v2_data else APP_DIR
    V2_MANAGED_INSTALL = v2_data is not None
else:
    APP_DIR = Path(__file__).resolve().parent
    RESOURCE_DIR = APP_DIR.parent
    BUNDLE_DIR = RESOURCE_DIR / "private" / "content-source" / "runtime"
    V2_MANAGED_INSTALL = False

LOCAL_APPDATA = Path(os.environ.get("LOCALAPPDATA", Path.home() / "AppData" / "Local"))
CONFIG_PATH = LOCAL_APPDATA / "GodiNavi" / "godinavi-config.json"
MAP_STORE = MapStore(RESOURCE_DIR)
ASSET_STORE = V2AssetStore(RESOURCE_DIR)


class PerformanceMetrics:
    """Low-overhead rolling performance log for field diagnostics."""
    ...
    def __init__(self, enabled, path=None, interval=10.0):
        ...
        self.process_cpu_at = time.process_time()

    def record(self, name, elapsed, cpu_elapsed=0.0):
        if not self.enabled:
            return
        with self.lock:
            item = self.metrics.setdefault(name, [0, 0.0, 0.0, 0.0])
            item[0] += 1
            item[1] += max(0.0, float(elapsed))
            item[2] = max(item[2], max(0.0, float(elapsed)))
            item[3] += max(0.0, float(cpu_elapsed))

    def flush_if_due(self, state="", force=False):
        if not self.enabled:
            return
        now = time.perf_counter()
        wall_seconds = now - self.started_at
        if not force and wall_seconds < self.interval:
            return
        process_cpu_now = time.process_time()
        process_cpu_seconds = max(0.0, process_cpu_now - self.process_cpu_at)
        logical_cpus = max(1, os.cpu_count() or 1)
        normalized_cpu = process_cpu_seconds / max(wall_seconds, 0.001) / logical_cpus * 100.0
        core_equivalent = process_cpu_seconds / max(wall_seconds, 0.001) * 100.0
        with self.lock:
            snapshot = self.metrics
            self.metrics = {}
            self.started_at = now
            self.process_cpu_at = process_cpu_now
        timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
        lines = [
            f"[{timestamp}] window={wall_seconds:.2f}s cpu={normalized_cpu:.1f}% "
            f"core_equivalent={core_equivalent:.1f}% logical_cpus={logical_cpus} {state}".rstrip()
        ]
        for name in sorted(snapshot):
            count, total, maximum, cpu_total = snapshot[name]
            average_ms = total / max(1, count) * 1000.0
            lines.append(
                f"  {name}: count={count} avg={average_ms:.2f}ms "
                f"max={maximum * 1000.0:.2f}ms total={total * 1000.0:.2f}ms "
                f"cpu_total={cpu_total * 1000.0:.2f}ms"
            )
        try:
...
OCR_LANGUAGE_BY_UI = {"KR": "ko", "JP": "ja", "EN": "en"}
...
def avoid_overlapping_rect(rect, obstacle, bounds):
    """Move rect inside bounds by the shortest axis needed to clear obstacle."""
    left, top, right, bottom = rect
    obs_left, obs_top, obs_right, obs_bottom = obstacle
    bound_left, bound_top, bound_right, bound_bottom = bounds
    if right <= obs_left or left >= obs_right or bottom <= obs_top or top >= obs_bottom:
        return left, top

    candidates = [
        (obs_left - (right - left), top),
        (obs_right, top),
        (left, obs_top - (bottom - top)),
        (left, obs_bottom),
    ]
    valid = [
        (x, y) for x, y in candidates
        if x >= bound_left and y >= bound_top
        and x + (right - left) <= bound_right
        and y + (bottom - top) <= bound_bottom
    ]
    if not valid:
        return left, top
    return min(valid, key=lambda point: abs(point[0] - left) + abs(point[1] - top))


DEFAULT_OCR_INTERVAL_MS = 250
NAME_OCR_REFRESH_SECONDS = 1.0


def normalized_ocr_interval_ms(value):
    try:
        interval = int(value)
    except (TypeError, ValueError):
        interval = DEFAULT_OCR_INTERVAL_MS
    return max(100, min(interval, DEFAULT_OCR_INTERVAL_MS))
...
DEFAULT_CONFIG = {
    ...
    "monster_list_overlay_orientation": "horizontal",
...
def load_config():
    ...
    try:
        ...
        config["ocr_interval_ms"] = normalized_ocr_interval_ms(config.get("ocr_interval_ms"))
...
FAVORITE_TEXTS = {
    "KR": {
        ...
        "search": "맵 이름 검색",
        "preset": "프리셋", "drag_hint": "마우스 휠로 투명도 조절",
    },
    "JP": {
        ...
        "search": "マップ名を検索",
        "preset": "プリセット", "drag_hint": "マウスホイールで透明度調整",
    },
    "EN": {
        ...
        "preset": "Preset", "drag_hint": "Use the mouse wheel to adjust opacity",
    },
}
...
def map_image_for_locale(map_record, ui_language):
    ...
    base_image = str(map_record.get("image", ""))
    if locale_key != "en" and base_image:
        base_path = Path(base_image)
        return str(base_path.parent / locale_key / base_path.name).replace("\\", "/")
    return base_image
...
def show_above_owner(window, owner_hwnd, x=None, y=None):
    ...
    owner_changed = bool(owner_hwnd and previous_owner != owner_hwnd)
    foreground = user32.GetForegroundWindow()
    current = foreground
    owner_group_foreground = False
    visited = set()
    while current and current not in visited:
        if current == hwnd or current == owner_hwnd:
            owner_group_foreground = True
            break
        visited.add(current)
        current = user32.GetWindow(current, GW_OWNER)
    foreign_foreground = bool(foreground and not owner_group_foreground)
...
class MapEngine:
    def __init__(self, master=None, shell_mode=False, on_calibration_confirmed=None):
        ...
        self.map_ocr_language = OCR_LANGUAGE_BY_UI.get(self.ui_language, "en")
        self.map_ocr_language_misses = 0
        self.map_ocr_language_miss_limit = 3
        ...
        self.favorite_preset_indicator_after_id = None
        self.favorite_lock_button = None
        self.favorite_header_window = None
        self.favorite_grip_window = None
        self.favorite_icon_images = {}
        self.favorite_location_buttons = []
        self.favorite_resize_origin = None
        self.favorite_overlay_scale_min = 0.63
        ...
        if self.ocr_backend_name == "windows":
            ...
            try:
                ...
                self.ocr = PaddleOcrBackend(RESOURCE_DIR / "content" / "ocr_models.pak" if V2_MANAGED_INSTALL else RESOURCE_DIR / "content-source" / "ocr_models")
        ...
        self.last_name_ocr_at = 0.0
        self.name_ocr_refresh_seconds = NAME_OCR_REFRESH_SECONDS
        self.last_coordinate_signature = None
        self.capture_coordinator = CaptureCoordinator(self.performance)
        self.capture_backend = self.capture_coordinator.backend
        self.closed = False
        ...
        tk.Label(header, text="GodiNavi", fg="#55c7f3", bg="#15191f", font=("Noto Sans", 11, "bold")).pack(side="left")
    ...
    def show_app_update_dialog(self, release):
        ...
        window.bind("<Escape>", lambda _event: close() or "break")
    ...
    def cycle_locale(self):
        ...
        self.map_ocr_language = OCR_LANGUAGE_BY_UI[self.ui_language]
        self.map_ocr_language_misses = 0
        self.last_name_ocr_at = 0.0
    ...
    def cycle_favorite_preset(self, direction=1):
        current = int(self.config.get("favorite_active_preset", 0))
        step = 1 if int(direction) >= 0 else -1
        index = current
        for distance in range(1, 5):
            candidate = (current + step * distance) % 4
            if self.favorite_entries(preset_index=candidate):
                index = candidate
                break
        index = self.activate_favorite_preset(index)
        self.rebuild_favorite_overlay()
        self.refresh_favorite_controls()
        self.show_favorite_preset_indicator(index)
        if hasattr(self, "root"):
            self.root.after_idle(self.focus_godius_after_portal_copy)
        return index

    def show_favorite_preset_indicator(self, index):
        """Temporarily show the selected preset on the lock tile.

        Resetting the timer on every click makes rapid navigation immediately
        display the newest preset for a full second.
        """
        if self.favorite_preset_indicator_after_id:
            try:
                self.root.after_cancel(self.favorite_preset_indicator_after_id)
            except (tk.TclError, ValueError):
                pass
        if self.favorite_lock_button and self.favorite_lock_button.winfo_exists():
            self.favorite_lock_button.configure(text=("I", "II", "III", "IV")[int(index)])
        self.favorite_preset_indicator_after_id = self.root.after(1000, self.restore_favorite_lock_label)

    def restore_favorite_lock_label(self):
        self.favorite_preset_indicator_after_id = None
        if self.favorite_lock_button and self.favorite_lock_button.winfo_exists():
            self.favorite_lock_button.configure(text="🔓" if self.favorite_overlay_editing else "🔒")

    def move_favorite_entry(self, index, direction):
        index = int(index)
        ...
        for language in FAVORITE_LANGUAGES:
            ...
            if index < len(locale_entries) and target < len(locale_entries):
                locale_entries[index], locale_entries[target] = locale_entries[target], locale_entries[index]
        save_config(self.config)
        self.rebuild_favorite_overlay()
        self.refresh_favorite_controls()
        return target
    ...
    def refresh_favorite_controls(self):
        ...
        if self.favorite_chrome_exists(getattr(self, "favorite_header_window", None)):
            labels = self.favorite_header_window.winfo_children()
            if labels:
                labels[0].configure(text=texts["drag_hint"])
    ...
    def open_favorite_dialog(self):
        ...
        title_label = tk.Label(
            ...
            font=("Noto Sans KR", 10, "bold"), padx=12,
        )
        title_label.pack(side="left", fill="y")
        close_button = tk.Button(
            header, text="×", bg="#2a2118", fg="#f1e5c7", activebackground="#6a3028",
            activeforeground="white", relief="flat", bd=0, width=4, font=("Noto Sans", 12, "bold"),
        )
        ...
        map_list = tk.Listbox(map_panel, exportselection=False, font=("Noto Sans KR", 10))
        map_list.configure(
        ...
        tk.Label(
            ...
            font=("Noto Sans KR", 9, "bold"), padx=2,
        ...
        registered_panel = ttk.Frame(body, style="Portal.TFrame")
        registered_panel.grid(row=4, column=0, columnspan=2, sticky="nsew")
        registered_panel.columnconfigure(0, weight=1)
        registered_panel.rowconfigure(0, weight=1)
        registered = ttk.Treeview(registered_panel, columns=columns, show="headings", selectmode="browse", height=6, style="Portal.Treeview")
        ...
        registered.grid(row=0, column=0, sticky="nsew")
        order_bar = tk.Frame(registered_panel, bg="#2a2118")
        order_bar.grid(row=1, column=0, sticky="e", pady=(5, 0))
        move_up_button = portal_button(order_bar, "▲")
        move_down_button = portal_button(order_bar, "▼")
        move_up_button.configure(width=3, padx=5, pady=3)
        move_down_button.configure(width=3, padx=5, pady=3)
        move_up_button.pack(side="left", padx=(0, 3))
        ...
        def refresh_action_states():
            ...
            entry_count = len(self.favorite_entries())
            move_up_button.configure(state="normal" if index is not None and index > 0 else "disabled")
            move_down_button.configure(
                state="normal" if index is not None and index < entry_count - 1 else "disabled"
            )
        ...
        def move_item(direction):
            ...
            if index is None:
                return
            target = self.move_favorite_entry(index, direction)
            if target is None:
                refresh_action_states()
                return
            editing_index["value"] = target
            refresh_registered()
            registered.selection_set(str(target))
            registered.focus(str(target))
            registered.see(str(target))
            refresh_action_states()
        ...
        move_up_button.configure(command=lambda: move_item(-1))
        move_down_button.configure(command=lambda: move_item(1))
        refresh_registered()
        refresh_preset_buttons()
        window.update_idletasks()
    ...
    def create_favorite_overlay(self):
        ...
        self.favorite_shell.bind("<MouseWheel>", self.favorite_overlay_wheel)
        win.withdraw()
        set_noactivate_toolwindow(win, click_through=False)
        win.attributes("-alpha", max(50, min(100, int(self.config.get("favorite_overlay_opacity_percent", 100)))) / 100.0)
        self.rebuild_favorite_overlay()
        return win
    ...
    def create_favorite_image_button(self, parent, text, command):
        ...
        button = tk.Button(
            ...
            font=("Noto Sans KR", max(8, round(9 * scale)), "bold"),
        ...
        button.bind("<MouseWheel>", self.favorite_overlay_wheel)
        return button
    ...
    def rebuild_favorite_overlay(self):
        ...
        for child in self.favorite_frame.winfo_children():
            child.destroy()
        self.favorite_location_buttons = []
        previous_button = self.create_favorite_image_button(
            self.favorite_frame,
            "◀",
            lambda: self.cycle_favorite_preset(-1),
        )
        previous_button.pack(side="left", padx=1, pady=1)
        next_button = self.create_favorite_image_button(
            self.favorite_frame,
            "▶",
            lambda: self.cycle_favorite_preset(1),
        )
        next_button.pack(side="left", padx=1, pady=1)
        for index, item in enumerate(self.favorite_entries()):
        ...
        self.favorite_lock_button = lock_button
        if self.favorite_overlay_editing:
            self.favorite_shell.configure(bg="#d8b15a", padx=1, pady=1)
    ...
    def copy_favorite(self, index):
        ...
        # Run after the button-release handler has completed so the overlay
        # cannot reclaim focus after we hand keyboard input back to the game.
        self.root.after_idle(self.focus_godius_after_portal_copy)

    def focus_godius_after_portal_copy(self):
        target = self.find_target()
        # Never steal focus back if the user has already switched to an
        # unrelated application before this idle callback runs.
        if target and not user32.IsIconic(target) and self.is_target_foreground():
    ...
    def restore_favorite_overlay_anchor(self, anchor):
        ...
        self.position_favorite_chrome()
        self.favorite_window.update_idletasks()

    def favorite_overlay_wheel(self, event):
        if not self.favorite_overlay_editing or not event.delta:
            return
        current = int(self.config.get("favorite_overlay_opacity_percent", 100))
        updated = max(50, min(100, current + (5 if event.delta > 0 else -5)))
        self.config["favorite_overlay_opacity_percent"] = updated
        self.favorite_window.attributes("-alpha", updated / 100.0)
        save_config(self.config)
        return "break"

    @staticmethod
    def favorite_chrome_exists(window):
        try:
            return bool(window and window.winfo_exists())
        except tk.TclError:
            return False

    def ensure_favorite_header_window(self):
        if self.favorite_chrome_exists(getattr(self, "favorite_header_window", None)):
            return self.favorite_header_window
        window = tk.Toplevel(self.root)
        window.overrideredirect(True)
        window.configure(bg="#d8b15a")
        label = tk.Label(
            window, text=FAVORITE_TEXTS[self.ui_language]["drag_hint"],
            bg="#5a4932", fg="#fff1c9", anchor="w", padx=8, pady=4,
            cursor="fleur", font=("Noto Sans KR", 8, "bold"),
        )
        label.pack(fill="both", expand=True, padx=1, pady=1)
        label.bind("<ButtonPress-1>", self.favorite_overlay_press)
        label.bind("<B1-Motion>", self.favorite_overlay_drag)
        label.bind("<ButtonRelease-1>", self.favorite_overlay_release)
        label.bind("<MouseWheel>", self.favorite_overlay_wheel)
        window.withdraw()
        set_noactivate_toolwindow(window, click_through=False)
        self.favorite_header_window = window
        return window

    def ensure_favorite_grip_window(self):
        if self.favorite_chrome_exists(getattr(self, "favorite_grip_window", None)):
            return self.favorite_grip_window
        window = tk.Toplevel(self.root)
        window.overrideredirect(True)
        window.configure(bg="#d8b15a")
        grip = tk.Canvas(
            window, width=16, height=16, bg="#35291c",
            highlightthickness=1, highlightbackground="#d8b15a", cursor="size_nw_se",
        )
        ...
        grip.bind("<ButtonPress-1>", self.favorite_resize_press)
        grip.bind("<B1-Motion>", self.favorite_resize_drag)
        grip.bind("<ButtonRelease-1>", self.favorite_resize_release)
        grip.bind("<MouseWheel>", self.favorite_overlay_wheel)
        window.withdraw()
        set_noactivate_toolwindow(window, click_through=False)
        self.favorite_grip_window = window
        return window

    def show_favorite_chrome(self):
        if self.favorite_overlay_editing:
            windows = (self.ensure_favorite_header_window(), self.ensure_favorite_grip_window())
        else:
            self.hide_favorite_edit_chrome()
            return
        self.position_favorite_chrome()
        for window in windows:
            show_above_owner(window, self.target_hwnd, window.winfo_x(), window.winfo_y())

    def hide_favorite_edit_chrome(self):
        for window in (
            getattr(self, "favorite_header_window", None), getattr(self, "favorite_grip_window", None),
        ):
            if self.favorite_chrome_exists(window):
                window.withdraw()

    def hide_favorite_chrome(self):
        for window in (
            getattr(self, "favorite_header_window", None), getattr(self, "favorite_grip_window", None),
            getattr(self, "favorite_lock_window", None),
        ):
            if self.favorite_chrome_exists(window):
                window.withdraw()

    def position_favorite_chrome(self):
        if not hasattr(self, "favorite_window") or not self.favorite_window.winfo_viewable():
            return
        left, top, right, bottom = self.favorite_bar_screen_rect()
        client = self.get_stable_client_rect()
        if not self.favorite_overlay_editing:
            return
        header = self.ensure_favorite_header_window()
        grip = self.ensure_favorite_grip_window()
        header_height = 28
        ...
        grip.geometry(f"+{right - grip_width}+{bottom - grip_height}")

    def favorite_resize_press(self, event):
        if not self.favorite_overlay_editing:
            return
        left, top, right, bottom = self.favorite_bar_screen_rect()
        self.favorite_resize_origin = (
            event.x_root, event.y_root, float(self.config.get("favorite_overlay_scale", 1.0)),
            max(1, right - left), max(1, bottom - top), self.favorite_overlay_anchor(),
        )
        try:
    ...
    def favorite_resize_drag(self, event):
        if not self.favorite_overlay_editing or not self.favorite_resize_origin:
            return
        start_x, start_y, start_scale, start_width, start_height, anchor = self.favorite_resize_origin
        horizontal_ratio = (start_width + event.x_root - start_x) / start_width
        vertical_ratio = (start_height + event.y_root - start_y) / start_height
        updated = max(
            self.favorite_overlay_scale_min,
            min(self.favorite_overlay_scale_max, round(start_scale * (horizontal_ratio + vertical_ratio) / 2.0, 2)),
        )
        if abs(updated - float(self.config.get("favorite_overlay_scale", 1.0))) < 0.01:
            return
        self.config["favorite_overlay_scale"] = updated
        self.favorite_icon_images.clear()
        self.rebuild_favorite_overlay()
        self.restore_favorite_overlay_anchor(anchor)
        self.position_favorite_chrome()

    def favorite_resize_release(self, event):
        if not self.favorite_resize_origin:
            return
        try:
    ...
    def show_favorite_overlay(self):
        if getattr(self, "overlays_temporarily_hidden", False):
            self.favorite_window.withdraw()
            self.hide_favorite_chrome()
            return
    ...
    def show_help(self):
        ...
        window.bind("<Escape>", lambda _event: window.destroy() or "break")
    ...
    def load_world_map_image(self):
        locale_key = {"KR": "ko", "JP": "ja"}.get(str(self.ui_language).upper())
        candidates = []
        if locale_key:
            candidates.append(f"maps/other/{locale_key}/worldmap.jpg")
        candidates.append("maps/other/worldmap.jpg")
        try:
            relative = next(value for value in candidates if ASSET_STORE.exists(value))
            with ASSET_STORE.open(relative) as stream, Image.open(stream) as source:
                self.world_map_original_image = source.convert("RGBA")
            self.world_map_render_key = None
            return True
        except Exception:
            self.world_map_original_image = None
    ...
    def set_active_map(self, map_record):
        ...
        for candidate in candidates:
            ...
            if candidate_path.is_absolute() and candidate_path.is_file():
                ...
                break
            if candidate and ASSET_STORE.exists(candidate):
                selected = candidate
                break
        resolved_path = str(selected.resolve()) if isinstance(selected, Path) else str(selected or "")
        if (
            self.active_map
        ...
        if isinstance(selected, Path):
            with Image.open(selected) as source:
                self.map_original_image = source.convert("RGBA")
        else:
            with ASSET_STORE.open(selected) as stream, Image.open(stream) as source:
                self.map_original_image = source.convert("RGBA")
    ...
    def draw_map_edit_chrome(self):
        ...
        self.map_canvas.create_text(
            ...
            font=("Noto Sans KR", 9, "bold"), tags="map_edit_chrome",
        )
    ...
    def capture_and_submit(self):
        ...
        if not client:
            return
        try:
            client_frame = self.capture_coordinator.capture_client(client)
        ...
        prepare_cpu_started = time.process_time()
        ...
        self.performance.record(
            ...
            time.process_time() - prepare_cpu_started,
        )

        now = time.monotonic()
        coordinate_signature = self.ocr_capture_signature(coordinate_capture)
        coordinate_changed = coordinate_signature != self.last_coordinate_signature
        recognize_name = (
            not self.active_map
            or bool(self.pending_map_id)
            or now - self.last_name_ocr_at >= self.name_ocr_refresh_seconds
            or not hasattr(self.ocr, "recognize_coordinates")
        )
        if not recognize_name and not coordinate_changed:
            self.performance.record("ocr_skipped_unchanged_coordinate", 0.0)
            return
        if recognize_name:
            self.last_name_ocr_at = now
        self.last_coordinate_signature = coordinate_signature

        def recognize_inputs():
            ocr_started = time.perf_counter()
            ocr_cpu_started = time.process_time()
            if recognize_name:
                language_key = self.map_ocr_language
                name_results, coordinate_text = self.ocr.recognize_map_language(
                    name_input, coordinate_input, language_key
                )
                result = (name_results, coordinate_text, True)
                metric_name = f"ocr_map_name_{language_key}_and_coordinate"
            else:
                result = ({}, self.ocr.recognize_coordinates(coordinate_input), False)
                metric_name = "ocr_coordinate_only"
            self.performance.record(
                metric_name,
                time.perf_counter() - ocr_started,
                time.process_time() - ocr_cpu_started,
            )
            return result

        self.ocr_running = True
        future = self.executor.submit(recognize_inputs)
        future.add_done_callback(lambda result: self.root.after(0, self.finish_ocr, result))

    @staticmethod
    def ocr_capture_signature(capture):
        """Cheap, noise-tolerant fingerprint for skipping unchanged OCR crops."""
        grayscale = capture.convert("L").resize((64, 16), Image.Resampling.BILINEAR)
        return grayscale.point(lambda value: value // 16).tobytes()
    ...
    def finish_ocr(self, future):
        ...
        if self.closed:
            return
        try:
            name_results, coordinate_text, recognized_name = future.result()
        except Exception as exc:
            for label in self.ocr_result_labels.values():
        ...
        if matched_map:
            self.map_ocr_language = matched_language
            self.map_ocr_language_misses = 0
            self.map_miss_started_at = None
            ...
            self.map_ocr_language_misses += 1
            if self.map_ocr_language_misses >= self.map_ocr_language_miss_limit:
                languages = ("ko", "ja", "en")
                current_index = languages.index(self.map_ocr_language)
                self.map_ocr_language = languages[(current_index + 1) % len(languages)]
                self.map_ocr_language_misses = 0
                self.last_name_ocr_at = 0.0
            self.pending_map_id = None
            self.pending_map_hits = 0
    ...
    def _accept_coordinate_text(self, coordinate_text):
        ...
        if len(numbers) >= 2:
            coordinate = (int(numbers[0]), int(numbers[1]))
            self.last_coordinate_at = time.monotonic()
            if coordinate != self.current_game_coordinate:
                self.current_game_coordinate = coordinate
                self.render_map()
    ...
    def position_map(self):
        ...
        if boundary_restricted:
            ...
            avoid_rect = getattr(self, "minimap_avoid_rect", None)
            if avoid_rect:
                x, y = avoid_overlapping_rect(
    ...
    def set_overlays_temporarily_hidden(self, hidden):
        self.overlays_temporarily_hidden = bool(hidden)
        if getattr(self, "overlays_temporarily_hidden", False):
            self.map_window.withdraw()
            self.favorite_window.withdraw()
            self.hide_favorite_chrome()
            self.region_window.withdraw()
            self.coordinate_region_window.withdraw()
            if self.ocr_guide_window and self.ocr_guide_window.winfo_exists():
                self.ocr_guide_window.withdraw()
            return
        if self.config.get("favorite_overlay_visible"):
            self.show_favorite_overlay()
        if self.world_map_mode:
            self.show_world_map()
        elif self.minimap_enabled and self.active_map:
            self.show_map()
    ...
    def tick(self):
        ...
        if self.overlays_temporarily_hidden:
            if self.map_window.winfo_viewable():
                self.map_window.withdraw()
            if self.favorite_window.winfo_viewable():
                self.favorite_window.withdraw()
            self.hide_favorite_chrome()
            self.root.after(50, self.tick)
            return
        ...
        self.performance.flush_if_due(
            state=(
                f"minimap={int(bool(self.minimap_enabled))} "
                f"map_active={int(bool(self.active_map))} "
                f"ocr_running={int(bool(self.ocr_running))}"
            )
        )
        self.root.after(50, self.tick)
    ...
    def quit(self):
        self.closed = True
        self.performance.flush_if_due(state="shutdown=1", force=True)
        if not self.shell_mode:
