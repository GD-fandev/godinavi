...
PARTY_MEMBER_COLORS = (
    "#A8E05F", "#55D66B", "#43D9A3", "#39D5D8",
    "#65C7F7", "#5687F5", "#9A7BF5", "#D86BEF",
    "#F178B6", "#F06565", "#F49A45", "#D6DEE8",
)
PARTY_SELF_COLOR = "#FFE14A"
PARTY_COLOR_NAMES = {
    "KR": ("연두", "초록", "민트", "청록", "하늘", "파랑", "보라", "자주", "분홍", "빨강", "주황", "은백색"),
    "JP": ("黄緑", "緑", "ミント", "ターコイズ", "水色", "青", "紫", "マゼンタ", "ピンク", "赤", "オレンジ", "シルバー"),
    "EN": ("Lime", "Green", "Mint", "Turquoise", "Sky Blue", "Blue", "Violet", "Magenta", "Pink", "Red", "Orange", "Silver"),
}
...
PARTY_TEXTS["KR"].update({
    ...
    "profile_delete": "삭제", "profile_edit": "수정", "profile_save": "저장", "profile_required": "프로필을 먼저 만들어 주세요",
    "profile_first": "첫 프로필 만들기", "ban_manage": "추방 관리", "ban_date": "추방일시",
    ...
    "display_color": "표시 색상 변경", "automatic": "자동", "map_member_names": "이름표시(미니맵)",
...
PARTY_TEXTS["JP"].update({
    ...
    "profile_delete": "削除", "profile_edit": "編集", "profile_save": "保存", "profile_required": "先にプロフィールを作成してください",
    "profile_first": "最初のプロフィールを作成", "ban_manage": "追放管理", "ban_date": "追放日時",
    ...
    "display_color": "表示色を変更", "automatic": "自動", "map_member_names": "名前表示(マップ上)",
...
PARTY_TEXTS["EN"].update({
    ...
    "profile_delete": "Delete", "profile_edit": "Edit", "profile_save": "Save", "profile_required": "Create a profile first",
    "profile_first": "Create your first profile", "ban_manage": "Manage bans", "ban_date": "Banned at",
    ...
    "display_color": "Change display color", "automatic": "Automatic", "map_member_names": "Name display (minimap)",
...
class PartyUI:
    def __init__(
        ...
        map_engine=None,
        center_message_callback: Callable[[str, int], None] | None = None,
    ):
        self.root = root
        self.config = config
        ...
        self.center_message_callback = center_message_callback or self.message_callback
        self.map_engine = map_engine
        self.members = {}
        self.session_color_assignments = {}
        self.member_color_preferences = self.config.setdefault("party_member_color_preferences", {})
        self.party_map_names_visible = bool(self.config.get("party_map_names_visible", True))
        self.member_color_menu_icons = []
        ...
        self.owner_overlays_suspended = False
    ...
    def open_settings(self):
        ...
        settings_button = tk.Button(
            ...
            relief="flat", bd=0, padx=9, pady=5, font=("Noto Sans KR", 11), cursor="hand2",
        )
    ...
    def _privacy_footer(self, body, notice):
        ...
        tk.Label(
            ...
            wraplength=0, font=("Noto Sans KR", 7),
    ...
    def _profiles(self):
        ...
        original = profiles if isinstance(profiles, list) else []
        profiles = []
    ...
    def _refresh_profile_selector(self, selector):
        ...
        if not profiles:
            variable.set(self.texts()["profile_required"])
            box.set(self.texts()["profile_required"])
            return
    ...
    def _add_field(self, body, row, label, variable, hint="", values=None):
        ...
        if hint:
            tk.Label(body, text=hint, bg=BG, fg="#a99570", anchor="w", font=("Noto Sans KR", 8)).grid(
    ...
    def _add_room_code_field(self, body, row, label, left_var, right_var, hint):
        ...
        clear_button = tk.Button(
            ...
            relief="flat", bd=0, padx=9, pady=3, font=("Noto Sans KR", 8, "bold"), cursor="hand2",
        )
    ...
    def _open_first_profile_dialog(self, profiles):
        texts = self.texts()
        window, _content, body, close = self._create_dialog(
            texts["profile_first"], 440, 345, show_header_close=False,
        )
        self.profile_window = window
        self._configure_form_style(window)
        body.configure(style="Party.TFrame", padding=(18, 16, 18, 14))
        body.columnconfigure(0, weight=1)
        name_var = tk.StringVar()
        job_var = tk.StringVar(value=self._job_label(JOBS[0]))
        sub_job_var = tk.StringVar(value=self._sub_job_label(SUB_JOBS[0]))
        name_entry = self._add_field(body, 0, texts["player_id"], name_var)
        self._add_compact_job_fields(body, 3, job_var, sub_job_var)
        footer = tk.Frame(body, bg=BG)
        ...
        def save_first():
            name = normalize_player_id(name_var.get())
            if not name:
                self._alert(texts["profile_first"], texts["invalid_player"])
                return
            profile = {
                "profile_id": uuid.uuid4().hex,
                "player_id": name,
                "job": self._canonical_job(job_var.get()),
                "sub_job": self._canonical_sub_job(sub_job_var.get()),
            }
            profiles.append(profile)
            self.config["party_profiles"] = profiles
            self.config["party_active_profile_id"] = profile["profile_id"]
            self.config["party_profile"] = {
                key: profile[key] for key in ("player_id", "job", "sub_job")
            }
            self.save_config()
            self._refresh_open_profile_selectors()
            close()

        self._button(footer, texts["profile_save"], save_first).pack(side="left", padx=(0, 6))
        self._button(footer, texts["cancel"], close).pack(side="left")
        window.after_idle(name_entry.focus_set)
    ...
    def open_profile_manager(self):
        ...
        profiles = self._profiles()
        if not profiles:
            self._open_first_profile_dialog(profiles)
            return
        texts = self.texts()
        window, _content, body, close = self._create_dialog(
    ...
    def _show_member_menu(self, event, member):
        ...
        if member_id == own_id:
            ...
            status = texts["enabled"] if self.party_map_names_visible else texts["disabled"]
            separator = "：" if self.language() == "JP" else " : " if self.language() == "KR" else ": "
            menu.add_command(
                label=f'{texts["map_member_names"]}{separator}{status}',
                command=self._toggle_party_map_names,
            )
        ...
        color_menu = tk.Menu(menu, tearoff=False, bg=FIELD, fg=TEXT, activebackground=HEADER, activeforeground="#fff4d2")
        selected = self.member_color_preferences.get(member_id)
        automatic_selected = selected not in PARTY_MEMBER_COLORS
        color_menu.add_command(
            label=texts["automatic"],
            command=lambda: self._set_member_display_color(member_id, None),
            background="#6b5537" if automatic_selected else FIELD,
            foreground="#ffffff" if automatic_selected else TEXT,
            activebackground="#806846", activeforeground="#ffffff",
        )
        color_menu.add_separator()
        self.member_color_menu_icons = []
        names = PARTY_COLOR_NAMES.get(self.language(), PARTY_COLOR_NAMES["EN"])
        for index, color in enumerate(PARTY_MEMBER_COLORS):
            is_selected = selected == color
            icon = self._color_menu_icon(color, is_selected)
            self.member_color_menu_icons.append(icon)
            label = f'{index + 1} · {names[index]}'
            color_menu.add_command(
                label=label, image=icon, compound="left",
                command=lambda value=color: self._set_member_display_color(member_id, value),
                background="#6b5537" if is_selected else FIELD,
                foreground="#ffffff" if is_selected else TEXT,
                activebackground="#806846", activeforeground="#ffffff",
            )
        menu.add_cascade(label=texts["display_color"], menu=color_menu)
    ...
    def _color_menu_icon(self, color, selected=False):
        image = Image.new("RGBA", (20, 16), (0, 0, 0, 0))
        draw = ImageDraw.Draw(image)
        if selected:
            draw.ellipse((7, 1, 19, 13), fill="#ffffff", outline="#17130f", width=1)
            draw.ellipse((9, 3, 17, 11), fill=color, outline=color, width=1)
        else:
            draw.ellipse((9, 2, 19, 12), fill=color, outline="#17130f", width=1)
        return ImageTk.PhotoImage(image, master=self.root)

    def _set_member_display_color(self, member_id, color):
        if color in PARTY_MEMBER_COLORS:
            self.member_color_preferences[member_id] = color
        else:
            self.member_color_preferences.pop(member_id, None)
    ...
    def _toggle_party_map_names(self):
        self.party_map_names_visible = not self.party_map_names_visible
        self.config["party_map_names_visible"] = self.party_map_names_visible
        self.save_config()
        self._apply_party_positions()
    ...
    def _toggle_tracking(self, member_id, key):
        ...
        if key == "position":
            ...
            if preferences["buff"]:
                self._start_current_remote_buff_timer(member_id)
            else:
                getattr(self, "remote_buff_timers", {}).pop(member_id, None)
            self._refresh_buff_bar()
    ...
    def _control_all_buff_tracking(self):
        ...
        for member in eligible:
            member_id = member.get("member_id")
            preferences = self.tracking_preferences.setdefault(
                member_id, {"position": True, "buff": False},
            )
    ...
    def _show_room_dialog(self, room_id):
        ...
        style.configure("Party.TFrame", background=BG)
        body.configure(style="Party.TFrame")
        tk.Label(body, text=texts["room_id"], bg=BG, fg="#a99570", font=("Noto Sans KR", 9)).pack(pady=(24, 6))
    ...
    def _on_client_event(self, event):
        ...
        if event_type == "room_snapshot":
            ...
            if isinstance(member, dict) and member.get("member_id"):
                ...
                self._store_party_member(member)
            ...
            member = event.get("member")
            if isinstance(member, dict) and member.get("member_id"):
                self._store_party_member(member, preserve_state=True)
    ...
    def _assign_party_member_colors(self):
        preferences = getattr(self, "member_color_preferences", {})
        own_id = getattr(getattr(self, "party_client", None), "member_id", None)
        ...
        if own_id in self.members:
            self.session_color_assignments.pop(own_id, None)
            self.members[own_id]["color"] = PARTY_SELF_COLOR
        used = {
            color for member_id, color in preferences.items()
            if member_id in active_ids and member_id != own_id and color in PARTY_MEMBER_COLORS
        }
        for member in members:
            member_id = member.get("member_id")
            if member_id == own_id:
                continue
            preferred = preferences.get(member_id)
            if preferred in PARTY_MEMBER_COLORS:
                self.session_color_assignments[member_id] = preferred
                member["color"] = preferred
        for member in members:
            if member.get("member_id") == own_id:
                continue
            if preferences.get(member.get("member_id")) not in PARTY_MEMBER_COLORS:
                self._assign_party_member_color(member, used)
                used.add(member.get("color"))

    def _store_party_member(self, member, preserve_state=False):
        """Store a server member record, then reapply this client's color choice."""
        member_id = member.get("member_id")
        if not member_id:
            return
    ...
    def _assign_party_member_color(self, member, used=None):
        member_id = member.get("member_id")
        if not member_id:
            return
        own_id = getattr(getattr(self, "party_client", None), "member_id", None)
        if member_id == own_id:
            self.session_color_assignments.pop(member_id, None)
            member["color"] = PARTY_SELF_COLOR
            return
        preferred = getattr(self, "member_color_preferences", {}).get(member_id)
        if preferred in PARTY_MEMBER_COLORS:
            self.session_color_assignments[member_id] = preferred
            member["color"] = preferred
            return
        ...
        if used is None:
            ...
            used.discard(color)
        if color not in PARTY_MEMBER_COLORS or color in used:
        ...
        for member_id, member in self.members.items():
            ...
            if state.get("map_id") and isinstance(state.get("x"), int) and isinstance(state.get("y"), int):
                positions.append({
                    ...
                    "show_name": self.party_map_names_visible,
                })
    ...
    def _refresh_member_list(self):
        ...
        for index, member in enumerate(members):
            ...
            map_text_item = map_canvas.create_text(
                0, 12, text="", fill=status_color, anchor="w", font=("Noto Sans KR", 9),
            )
            coordinate_label = tk.Label(
                location_frame, text="", bg=FIELD, fg="#d8b15a", anchor="e",
                padx=0, pady=0, font=("Noto Sans KR", 9),
            )
    ...
    def _ensure_chat_overlay(self):
        ...
        rect = self.client_rect()
        if not rect:
            # Do not create the overlay at an arbitrary monitor coordinate
            # before the Godius client rectangle is available.
            return False
        ...
        saved_offset_x = self.config.get("party_chat_overlay_offset_x")
        saved_offset_y = self.config.get("party_chat_overlay_offset_y")
        legacy_position = self.config.pop("party_chat_overlay_position", None)
        if saved_offset_x is None or saved_offset_y is None:
            if isinstance(legacy_position, (list, tuple)) and len(legacy_position) == 2:
                saved_offset_x = int(legacy_position[0]) - left
                saved_offset_y = int(legacy_position[1]) - top
            else:
                saved_offset_x = 24
                saved_offset_y = 24
        offset_x = max(0, min(int(saved_offset_x), max(0, right - left - width)))
        offset_y = max(0, min(int(saved_offset_y), max(0, bottom - top - height)))
        x, y = left + offset_x, top + offset_y
        window._party_offset = (offset_x, offset_y)
        self.config["party_chat_overlay_offset_x"] = offset_x
        self.config["party_chat_overlay_offset_y"] = offset_y
        if legacy_position is not None or offset_x != saved_offset_x or offset_y != saved_offset_y:
            self.save_config()
        ...
        control_style = {
            ...
            "relief": "flat", "bd": 0, "padx": 5, "pady": 1, "font": ("Noto Sans", 8, "bold"),
        }
        ...
        input_entry = tk.Entry(
            ...
            relief="flat", bd=0, font=("Noto Sans KR", 9),
        )
        ...
        label = tk.Text(
            ...
            font=("Noto Sans KR", self._chat_overlay_font_size()), cursor="arrow",
        )
    ...
    def _sync_chat_overlay(self):
        should_show = self.joined and self.chat_overlay_enabled and not self.owner_overlays_suspended
        if not should_show:
            ...
            return
        if not self._ensure_chat_overlay():
            return
    ...
    def _adjust_chat_overlay_font(self, delta):
        ...
        if self._window_exists(self.chat_overlay_label):
            self.chat_overlay_label.configure(font=("Noto Sans KR", self._chat_overlay_font_size()))
        self._apply_chat_overlay_lock_state(save=False)
    ...
    def _save_chat_overlay_geometry(self):
        ...
        if offset:
            self.config["party_chat_overlay_offset_x"] = int(offset[0])
            self.config["party_chat_overlay_offset_y"] = int(offset[1])
        self.config.pop("party_chat_overlay_position", None)
    ...
    def _start_current_remote_buff_timer(self, member_id):
        member = self.members.get(member_id) or {}
        state = member.get("state") or {}
        buff_key = state.get("buff") if state.get("buff_present") else None
        if buff_key in {"fire", "ice"}:
            self._update_remote_buff_timer(member_id, buff_key)

    def _tick_remote_buff_timers(self):
        try:
            now = time.monotonic()
            self._restart_active_expired_buff_timers(now)
            self._update_buff_bar_countdowns(now)
            self.root.after(500, self._tick_remote_buff_timers)
        except tk.TclError:
            return

    def _restart_active_expired_buff_timers(self, now):
        for member_id, timer in list(self.remote_buff_timers.items()):
            if timer.get("end_time", 0) > now:
                continue
            member = self.members.get(member_id) or {}
            state = member.get("state") or {}
            buff_key = state.get("buff") if state.get("buff_present") else None
            if self.personal_buff_timer_visible and buff_key in {"fire", "ice"}:
                duration = self.personal_buff_durations.get(buff_key, 598)
                self.remote_buff_timers[member_id] = {
                    "buff": buff_key,
    ...
    def open_personal_buff_timer(self):
        ...
        tk.Checkbutton(
            ...
            font=("Noto Sans KR", 9, "bold"), anchor="w",
        ...
        tk.Label(
            ...
            wraplength=285, anchor="w", font=("Noto Sans KR", 8),
    ...
    def _refresh_buff_bar(self):
        ...
        previous_offset = getattr(window, "_party_offset", None) if was_visible else None
        ...
        for member in members:
            ...
            card.create_text(
                ...
                font=("Noto Sans KR", max(7, round(9 * scale)), "bold"), anchor="center",
            )
        ...
        if rect and isinstance(previous_offset, (list, tuple)) and len(previous_offset) == 2:
            left, top, _right, _bottom = rect
            x, y = left + int(previous_offset[0]), top + int(previous_offset[1])
        ...
        if self.owner_overlays_suspended:
            window.withdraw()
            return
        owner = self.owner_hwnd()
        if owner:
            # Tk applies geometry asynchronously. Reading winfo_x/y here can
            # briefly return 0,0 during startup and overwrite the restored
            # position when the native owner attachment moves the window.
            attach_above(window, owner, int(x), int(y))
        else:
            window.deiconify()
        if self.buff_bar_adjusting:
    ...
    def _forget_session(self):
        ...
        color_preferences = getattr(self, "member_color_preferences", None)
        if color_preferences:
            color_preferences.clear()
            self.session_color_assignments.clear()
            changed = True
        if changed:
            self.save_config()
    ...
    def _restore_session(self):
        ...
        if self.party_client.resume_session(session):
            ...
            self.center_message_callback(self.texts()[key], 2400)
            self.root.after_idle(self._sync_chat_overlay)
        else:
            self._forget_session()
    ...
    def set_owner_overlays_available(self, available):
        """Hide owner-bound overlays while the Godius window is unavailable."""
        overlay_windows = (
            self.buff_bar_window,
            self.buff_bar_header_window,
            self.buff_bar_grip_window,
            self.buff_bar_lock_window,
            self.chat_overlay_window,
        )
        if not available:
            if self.owner_overlays_suspended:
                return
            self.owner_overlays_suspended = True
            for window in overlay_windows:
                if self._window_exists(window) and window.winfo_viewable():
                    window.withdraw()
            return

        if not self.owner_overlays_suspended:
            return
        self.owner_overlays_suspended = False
        self._sync_chat_overlay()
        self._refresh_buff_bar()
    ...
    def _confirm(self, title, message, show_header_close=True, confirm_text=None, cancel_text=None,
        ...
        message_label = tk.Label(
            ...
            font=("Noto Sans KR", font_size), padx=8, pady=12,
        )
    ...
    def _alert(self, title, message):
        ...
        tk.Label(
            ...
            wraplength=370, font=("Noto Sans KR", 10), padx=8, pady=18,
    ...
    def _show_dismissible_notice(self, title, message, checkbox_text, config_key, hidden_value, width, height):
        ...
        tk.Checkbutton(
            ...
            font=("Noto Sans KR", 9), cursor="hand2", highlightthickness=0, bd=0,
        ).pack(side="left")
        ...
        # Bundled fonts can have a taller line box than the platform font that
        # was used when these dialogs had fixed heights. Measure the rendered
        # text and grow the modal when possible; otherwise keep the footer
        # visible and make only the message body scrollable.
        window.update_idletasks()
        actual_width = max(320, window.winfo_width())
        message_width = max(260, actual_width - 54)
        message_label.configure(wraplength=message_width)
        message_canvas.itemconfigure(message_item, width=message_width)
        window.update_idletasks()
        desired_height = 40 + message_label.winfo_reqheight() + footer.winfo_reqheight() + 66
        rect = self.client_rect()
        max_height = (rect[3] - rect[1] - 24) if rect else (window.winfo_screenheight() - 48)
        target_height = min(max(height, desired_height), max_height)
        if target_height != window.winfo_height():
    ...
    def _button(self, parent, text, command):
        return tk.Button(
            ...
            padx=14, pady=7, font=("Noto Sans KR", 9, "bold"), cursor="hand2",
        )
    ...
    def _create_dialog(self, title, width, height, show_header_close=True, modal=True, position_key=None):
        ...
        title_label = tk.Label(
            header, text=title, bg=HEADER, fg="#ffe3a1", font=("Noto Sans KR", 10, "bold"), padx=12,
        )
        title_label.pack(side="left", fill="y")
        close_button = tk.Button(
            header, text="×", bg=BG, fg=TEXT, activebackground="#6a3028", activeforeground="white",
            relief="flat", bd=0, width=4, font=("Noto Sans", 12, "bold"),
        )
