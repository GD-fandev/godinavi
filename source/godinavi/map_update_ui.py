...
TEXTS = {
    "KR": {
        ...
        "prompt_message": "새로운 맵 & 몬스터 자료 업데이트가 있습니다.\n지금 받으시겠습니까?",
    ...
    "JP": {
        ...
        "prompt_message": "新しいマップ＆モンスターデータがあります。\n今すぐダウンロードしますか？",
    ...
    "EN": {
        ...
        "prompt_message": "New map and monster data is available.\nWould you like to download it now?",
        "prompt_yes": "Yes", "prompt_tomorrow": "Remind me tomorrow",
    },
}


def map_reminder_path():
    return Path(os.environ.get("LOCALAPPDATA", Path.cwd())) / "GodiNavi" / "map-update-reminder.json"
...
class MapUpdateUI:
    def __init__(self, root, app_dir, language_provider, state_callback=None, installed_callback=None, target_rect_provider=None, owner_hwnd_provider=None, prompt_decision_callback=None, before_install_callback=None, preview_decision_callback=None):
        self.root = root
        ...
        self.before_install_callback = before_install_callback
    ...
    def install(self):
        ...
        if self.busy or not self.manifest:
            return
        if self.before_install_callback:
            self.before_install_callback()
        self.busy = True
    ...
    def show_update_prompt(self, force=False, preview=False):
        if not force and (self.state != "available" or map_update_prompt_is_snoozed()):
            return
        ...
        title.pack(fill="x")
        tk.Label(frame, text=text["prompt_message"], bg="#17130f", fg="#f1e5c7", anchor="w", justify="left", padx=8, pady=10, font=(family, 10)).pack(fill="both", expand=True)
        ...
        _owner, bounds = place_modal(win, minimum_width=500, minimum_height=220, position_key="map_update_prompt")
        bind_modal_drag(win, (header, title), lambda: bounds, position_key="map_update_prompt")
    ...
    def open(self):
        ...
        self.labels["title"] = tk.Label(header, bg="#5a4932", fg="#ffe09a", anchor="w", padx=12, pady=9, font=("Noto Sans KR", 13, "bold"))
        ...
        for key in ("current", "latest", "size"):
            ...
            self.labels[key + "_name"] = tk.Label(row, bg="#17130f", fg="#bda982", anchor="w", width=19, font=("Noto Sans KR", 9))
            self.labels[key + "_name"].pack(side="left")
            self.labels[key] = tk.Label(row, bg="#2a2118", fg="#fff1c9", anchor="w", padx=9, pady=4, width=24, font=("Noto Sans KR", 9, "bold"))
            self.labels[key].pack(side="left", fill="x", expand=True)
        self.labels["status"] = tk.Label(frame, bg="#17130f", fg="#f1e5c7", anchor="w", justify="left", wraplength=410, pady=12, font=("Noto Sans KR", 9))
        ...
        buttons.pack(fill="x")
        self.primary_button = tk.Button(buttons, relief="flat", bg="#6b5537", fg="#fff1c9", activebackground="#806846", activeforeground="#ffffff", padx=18, pady=6, font=("Noto Sans KR", 9, "bold"))
        self.primary_button.pack(side="right")
        self.secondary_button = tk.Button(buttons, command=self.close, relief="flat", bg="#2a2118", fg="#f1e5c7", activebackground="#443422", activeforeground="#ffffff", padx=18, pady=6, font=("Noto Sans KR", 9))
