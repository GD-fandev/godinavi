...
from .window_attachment import attach_above, focus_native_window
...
if getattr(sys, "frozen", False):
    ...
    RESOURCE_DIR = materialize_runtime_assets()
    V2_INSTALL_ROOT = APP_DIR.parent if APP_DIR.name.lower() == "data" else (APP_DIR.parents[1] if APP_DIR.name.lower() == "app" and APP_DIR.parent.name.lower() == "data" else None)
else:
    APP_DIR = Path(__file__).resolve().parent
    RESOURCE_DIR = Path(__file__).resolve().parents[2] / "private" / "content-source" / "runtime"
    V2_INSTALL_ROOT = None
...
DEFAULT_BUFF_CONFIG_JSON = r'''{
  ...
  "detect_text_obstruction_bypass_similarity": 0.96,
...
APP_CONFIG_DIR = Path(os.environ.get("LOCALAPPDATA", APP_DIR)) / "GodiNavi" / "buff_timer"
APP_CONFIG_PATH = APP_CONFIG_DIR / "config.json"
LEGACY_CONFIG_PATH = (V2_INSTALL_ROOT or APP_DIR) / "config.json"
ROAMING_LEGACY_CONFIG_PATH = Path(os.environ.get("APPDATA", APP_DIR)) / "GodiusCrystalBuffTimer" / "config.json"
...
def load_config():
    ...
    legacy_path = next((path for path in (LEGACY_CONFIG_PATH, ROAMING_LEGACY_CONFIG_PATH) if path.exists() and path != bundled_config), None)
    if not appdata_exists and legacy_path:
...
def monitor_for_bounds(x, y, width, height):
    rect = wintypes.RECT(int(x), int(y), int(x) + max(1, int(width)), int(y) + max(1, int(height)))
    return int(user32.MonitorFromRect(ctypes.byref(rect), MONITOR_DEFAULTTONEAREST) or 0)
...
class BuffTimerApp:
    def __init__(self, master, config, save_callback=None, target_provider=None, on_calibration_confirmed=None, performance=None, capture_coordinator=None):
        self.config = config
        ...
        self.owns_capture_coordinator = capture_coordinator is None
        self.closed = False
        ...
        self.detected_ignore_template = None
        self.buffs = self.load_buffs()
        self.absent_template = self.load_plain_template("absent_template_path")
        self.ignore_templates = self.load_ignore_templates()
        self.manual_position = False
        self.last_timer_geometry = ""
        self.last_timer_monitor = None
        self.last_timer_position = None
        self.capture_backend = capture_coordinator.backend if capture_coordinator is not None else "Desktop Duplication"
        ...
        if self.capture_coordinator is None and dxcam is not None:
            try:
                ...
                self.desktop_camera = None
        if self.capture_coordinator is None and self.desktop_camera is None:
            self.capture_backend = "ImageGrab fallback"

        self.root = tk.Toplevel(master)
        # An OFF timer must never be mapped at Tk's default/stale absolute
        # coordinates while the remaining helper windows are being created.
        if not getattr(self, "timer_visible", True):
            self.root.withdraw()
        ...
        self.root.geometry(f"{self.width}x{self.height}{start_x:+d}{start_y:+d}")
        ...
        for widget in (self.timer_label, self.text_label):
            widget.bind("<Double-Button-1>", lambda event: self.manual_hidden_buff_start(event, "ice"))
            widget.bind("<Double-Button-3>", lambda event: self.manual_hidden_buff_start(event, "fire"))
    ...
    def create_calibration_guide_window(self):
        ...
        correct_title = tk.Label(
            ...
            anchor="w", font=("Noto Sans KR", 9, "bold"),
        )
        ...
        for symbol, delta in (("−", -2), ("+", 2)):
            button = tk.Button(
                ...
                relief="flat", bd=0, cursor="hand2", font=("Noto Sans KR", 12, "bold"),
            )
    ...
    def create_resize_window(self):
        ...
        self.resize_tooltip_label = tk.Label(
            ...
            padx=10, pady=7, justify="left", font=("Noto Sans KR", 9),
        )
    ...
    def quit_app(self):
        if self.closed:
            return
        self.closed = True
        if self.owns_capture_coordinator and self.desktop_camera is not None:
            try:
    ...
    def render_timer_image(self, text, color, buff_key=None):
        ...
        self.last_display_render_key = ("timer", self.width, self.height, text, color, buff_key)
        self.restore_timer_position_after_render()
    ...
    def render_no_buff_image(self):
        ...
        self.text_image = ImageTk.PhotoImage(text_canvas)
        self.text_label.configure(image=self.text_image)
        self.last_display_render_key = ("none", self.width, self.height)
        self.restore_timer_position_after_render()

    def restore_timer_position_after_render(self):
        position = getattr(self, "last_timer_position", None)
        if position is None:
            return
        # Flushing the PhotoImage/layout update first prevents Tk from
        # repainting either transparent layer at its pre-transition DPI-
        # virtualized coordinates for one frame.
        self.root.update_idletasks()
        self.text_window.update_idletasks()
        self.move_timer_layers_native(*position)
    ...
    def keep_timer_visible(self):
        ...
        self.root.attributes("-alpha", self.display_icon_opacity)
        if self.target_hwnd:
            if not self.root.winfo_viewable() or self.timer_owner_hwnd != self.target_hwnd:
                attach_above(self.root, self.target_hwnd)
                self.timer_owner_hwnd = self.target_hwnd
            icon_hwnd = root_hwnd(self.root)
            if not self.text_window.winfo_viewable() or self.text_owner_hwnd != icon_hwnd:
    ...
    def clamp_window_position(self, x, y, width, height):
        ...
        if client and self.config.get("timer_attach_to_client", True):
            ...
            # Use the complete virtual desktop as the safe fallback instead of
            # assuming the primary monitor begins at 0,0.
            left = user32.GetSystemMetrics(76)  # SM_XVIRTUALSCREEN
            top = user32.GetSystemMetrics(77)   # SM_YVIRTUALSCREEN
            right = left + max(1, user32.GetSystemMetrics(78))
            bottom = top + max(1, user32.GetSystemMetrics(79))
    ...
    def update_timer_attached_position(self):
        ...
        if not client:
            return
        raw_x = client[0] + int(self.config.get("timer_offset_x", 18))
        raw_y = client[1] + int(self.config.get("timer_offset_y", 46))
        x, y = self.clamp_timer_position(raw_x, raw_y)
        if (x, y) != (raw_x, raw_y):
            self.set_timer_position(x, y)
        self.apply_timer_geometry(x, y)
    ...
    def apply_timer_geometry(self, x, y):
        x, y = self.clamp_timer_position(x, y)
        self.last_timer_position = (x, y)
        geometry = f"{self.width}x{self.height}{x:+d}{y:+d}"
        monitor = monitor_for_bounds(x, y, self.width, self.height)
        monitor_changed = bool(
            monitor and self.last_timer_monitor and monitor != self.last_timer_monitor
        )
        if monitor_changed:
            # Tk uses two transparent native windows for the icon and text.
            # Re-map both layers together when crossing monitors so Windows
            # discards the old layered surfaces instead of leaving one behind.
            self.root.withdraw()
            self.text_window.withdraw()
        geometry_changed = geometry != self.last_timer_geometry
        if geometry_changed:
        ...
        if monitor_changed:
            ...
            self.text_window.update_idletasks()
            self.last_display_render_key = None
            if self.timer_visible and not self.overlays_temporarily_hidden:
                self.root.deiconify()
                self.text_window.deiconify()
        # Tk geometry coordinates can be DPI-virtualized when an existing
        # layered window crosses to a monitor with a different scale. Move
        # both native layers to the physical client-relative coordinates so
        # they cannot split or leave a stale layer behind on the old monitor.
        self.move_timer_layers_native(x, y)
        if monitor:
            self.last_timer_monitor = monitor
        self.last_timer_geometry = geometry

    def move_timer_layers_native(self, x, y):
        move_flags = SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE
        user32.SetWindowPos(root_hwnd(self.root), 0, int(x), int(y), 0, 0, move_flags)
        user32.SetWindowPos(root_hwnd(self.text_window), 0, int(x), int(y), 0, 0, move_flags)
    ...
    def can_manually_start_hidden_buff(self):
        return (
    ...
    def manual_hidden_buff_start(self, _event, buff_key):
        if buff_key not in {"ice", "fire"} or not self.can_manually_start_hidden_buff():
            return None
        self.start_timer(buff_key)
        self.detect_hits.clear()
        self.absent_hits = 0
        self.missing_hits = 0
    ...
    def focus_target_window(self):
        if not self.target_hwnd or not get_client_screen_rect(self.target_hwnd):
            self.target_hwnd = find_window_by_title(self.config.get("window_title", ""))
        if not self.target_hwnd:
            self.target_hwnd = find_window_by_process_name(self.config.get("process_name", "Godius.exe"))
        if self.target_hwnd:
            focus_native_window(self.target_hwnd)
    ...
    def ignored_template_match(self, capture):
        threshold = float(self.config.get("ignore_template_threshold", 0.9))
        min_score_gap = max(0.0, float(self.config.get("ignore_template_min_score_gap", 0.08)))
        best_name = None
        best_score = 0.0
        for name, template in getattr(self, "ignore_templates", []):
            score = self.score_aligned_ignore_template(capture, template)
            if score > best_score:
                best_name = name
                best_score = score
        # Dark slot backgrounds make unrelated red icons look deceptively close
        # by absolute pixel difference. Only reject when the negative template
        # is also clearly closer than every real buff template.
        best_buff_score = max(
            (
                self.score_by_pixel_difference(capture, buff["template"])
                for buff in getattr(self, "buffs", [])
            ),
            default=0.0,
        )
        if best_name and best_score >= threshold and best_score - best_buff_score >= min_score_gap:
            return best_name, best_score
        return None

    def score_aligned_ignore_template(self, capture, template):
        """Find a negative icon inside the slot without stretching slot borders."""
        cap = np.asarray(capture.convert("RGB"), dtype=np.float32)
        cap_height, cap_width = cap.shape[:2]
        base_size = min(template.size)
        sizes = {
            max(1, round(base_size * scale))
            for scale in (0.90, 0.95, 1.0, 1.05, 1.10)
        }
        best_score = 0.0
        for size in sorted(sizes):
            if size > cap_width or size > cap_height:
                continue
            candidate = template.resize((size, size), Image.Resampling.LANCZOS)
            candidate_arr = np.asarray(candidate, dtype=np.float32)
            for top in range(cap_height - size + 1):
                for left in range(cap_width - size + 1):
                    patch = cap[top:top + size, left:left + size]
                    score = 1.0 - (float(np.mean(np.abs(patch - candidate_arr))) / 255.0)
                    best_score = max(best_score, score)
        return best_score

    def capture_client_frame(self, client_rect):
        if self.capture_coordinator is not None:
            return self.capture_coordinator.capture_client(client_rect)
    ...
    def detect_buff_present(self):
        ...
        detect_cpu_started = time.process_time()
        try:
            return self._detect_buff_present()
        finally:
            if self.performance is not None:
                self.performance.record(
                    "buff_detect",
                    time.perf_counter() - detect_started,
                    time.process_time() - detect_cpu_started,
                )

    def _detect_buff_present(self):
        self.detected_ignore_template = None
        self.last_absent_score = None
        ...
        try:
            ...
            if hide_overlay and self.timer_visible:
                self.root.deiconify()
                self.text_window.deiconify()
        if capture is None:
        ...
        ignored_match = self.ignored_template_match(capture)
        if ignored_match:
            ignored_name, ignored_score = ignored_match
            self.detected_ignore_template = ignored_name
            self.update_debug_capture_preview(
                capture,
                bbox,
                f"ignored: {ignored_name} {ignored_score:.3f}",
            )
            return None
        ...
        if best_key and frame_scores.get(best_key, 0.0) < frame_threshold:
            ...
            best_key = None
        if best_key and self.is_text_obstructed(
            full_scores.get(best_key, 0.0),
            obstruction_scores.get(best_key, 0.0),
        ):
    ...
    def is_text_obstructed(self, full_score, obstruction_score):
        threshold = float(self.config.get("detect_text_obstruction_threshold", 0.55))
        bypass_similarity = float(self.config.get("detect_text_obstruction_bypass_similarity", 0.96))
        # A capture that is nearly identical to the complete buff template is
        # the icon itself, even when its bright outer flames resemble glyphs.
        return obstruction_score >= threshold and full_score < bypass_similarity
    ...
    def handle_auto_detect(self):
        ...
        if not getattr(self, "timer_visible", True):
            # Party presence observation remains active while the personal
            # timer is OFF, but it must not start or retain a personal
            # countdown that could later render through a hidden window.
            self.detect_hits.clear()
            self.absent_hits = 0
            self.missing_hits = 0
            self.active_buff = None
    ...
    def update_display(self):
        if self.end_time is None:
            if self.last_display_render_key != ("none", self.width, self.height):
                self.render_no_buff_image()
            self.root.attributes("-alpha", self.display_icon_opacity)
            return
        ...
        render_key = ("timer", self.width, self.height, label, color, self.active_buff)
        if self.last_display_render_key != render_key:
    ...
    def tick(self):
        ...
        if self.timer_visible:
            ...
            # Defense in depth for Tk/Win32 calls that may have mapped either
            # layer while refreshing images or window styles.
            self.root.withdraw()
            self.text_window.withdraw()
        try:
            self.root.after(100, self.tick)
